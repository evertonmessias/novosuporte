...
/**
 * Encode the Unnovosuporte values to be used in the URI.
 *
 * @since 1.5.0
 ...
 * @return string String with Unnovosuporte encoded for URI.
 */
function utf8_uri_encode( $utf8_string, $length = 0, $encode_ascii_characters = false ) {
	$unnovosuporte        = '';
	$values         = array();
	$num_octets     = 1;
	$unnovosuporte_length = 0;
	...
	for ( $i = 0; $i < $string_length; $i++ ) {
		...
		if ( $value < 128 ) {
			...
			if ( $length && ( $unnovosuporte_length + $encoded_char_length ) > $length ) {
				break;
			}
			$unnovosuporte        .= $encoded_char;
			$unnovosuporte_length += $encoded_char_length;
		} else {
			if ( count( $values ) == 0 ) {
				if ( $value < 224 ) {
					$num_octets = 2;
			...
			if ( $length && ( $unnovosuporte_length + ( $num_octets * 3 ) ) > $length ) {
				break;
			}
			if ( count( $values ) == $num_octets ) {
				for ( $j = 0; $j < $num_octets; $j++ ) {
					$unnovosuporte .= '%' . dechex( $values[ $j ] );
				}

				$unnovosuporte_length += $num_octets * 3;
...
/**
 * Converts invalid Unnovosuporte references range to valid range.
 *
 * @since 4.3.0
 *
